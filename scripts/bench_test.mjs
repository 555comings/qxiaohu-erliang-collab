...
test('memory_bench smoke run writes summary artifacts', async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'memory-bench-test-'));
  const outDir = path.join(tempRoot, 'out');
  const workspaceDir = path.join(tempRoot, 'workspaces');

  try {
    const run = await runNode([
      'scripts/memory_bench.mjs',
      '--scenario', 'startup-read-s',
      '--mode', 'warm',
      '--runs', '1',
      '--warmup', '0',
      '--out', outDir,
    ...
    assert.equal(summary.bench, 'memory');
    assert.equal(summary.scenarioSummaries['startup-read-s@warm'].okRuns, 1);
    assert.match(report, /startup-read-s/);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});
